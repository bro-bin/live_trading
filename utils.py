...
def get_basket_qty(live_prices: dict, tolerance: float = 1.0) -> dict:
    ...
    Raises:
        ValueError: 필수 종목 가격 데이터가 없거나 삼성카드 비중이 0인 경우
    """
    ...
    for stock_name, comp_info in ETF_COMPOSITION.items():
        ...
        # ✅ 수정: live_prices에서 price와 code 추출
    ...
    samsung_card_row = df[df['종목명'] == '삼성카드'].iloc[0]
    ...
    # 3단계: 각 종목별 수량 계산 (✅ 수정: 종목코드를 키로 사용)
    basket_dict = {}
    
    for _, row in df.iterrows():
        stock_name = row['종목명']
        stock_code = row['종목코드']  # ✅ 추가
