...
def get_optimal_basket(tolerance=1.0, verbose=True):
    """
    ...
    Returns:
        tuple: (optimal_portfolio_df, total_cost, max_error, target_df, original_total_cap)
    """
    global _client
    ...
    return optimal_portfolio, optimal_cost, max_error, target_df, total_cap
...
if __name__ == "__main__":
    try:
        ...
        while True:
            try:
                print(f"\n⏰ {time.strftime('%Y-%m-%d %H:%M:%S')} - 최적 바스켓 계산 중...")
                
                # 최적 바스켓 계산 (실시간 가격으로)
                result = get_optimal_basket(tolerance=1.0, verbose=True)
