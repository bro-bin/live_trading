...
# ##############################################################################
# KISWebSocketClient 클래스 및 관련 함수 (from get_rate.py)
# ##############################################################################
...
# ##############################################################################
# 포트폴리오 계산 관련 함수 (from make_basket.py)
# ##############################################################################
...
def create_minimum_cost_portfolio(target_df, tolerance=1.0):
    ...
    for samsung_card_quantity in range(1, 21):
        ...
        print(f"\n--- 삼성카드 {samsung_card_quantity}개 기준으로 계산 (예상 총액: {total_portfolio_value:,.0f}원) ---")
        ...
        if portfolio_df['오차절댓값'].le(tolerance).all():
            print(f"✅ 성공! 모든 종목이 허용 오차 {tolerance:.1f}% 이내입니다. (실제 총 투자금액: {actual_total_cost:,}원)")
